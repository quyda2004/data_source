...
### FastAPI + Uvicorn
- **traffic** (`backend/app/main.py`, `backend/app/api/v1/`) — [backend/app/main.py](https://github.com/quyda2004/check/blob/main/backend/app/main.py)
- **badminton** (`backend/api/main.py`) — [backend/api/main.py](https://github.com/quyda2004/AI_badminton/blob/main/backend/api/main.py)
- **ai-coworker** (`app/main.py`, `app/api/routes/`) — [app/main.py](https://github.com/quyda2004/AI_coworker/blob/main/app/main.py)

### WebSocket streaming (native FastAPI)
- **traffic**: 4 kênh — [backend/app/api/v1/api_vehicles_frames.py](https://github.com/quyda2004/check/blob/main/backend/app/api/v1/api_vehicles_frames.py)
...
- **traffic** (`backend/app/api/v1/api_auth.py`) — [backend/app/api/v1/api_auth.py](https://github.com/quyda2004/check/blob/main/backend/app/api/v1/api_auth.py)
  - Payload: `sub = user_id`, `role_id`
  - WS truyền token qua query param `?token=...` (không có Authorization header)
- **badminton** (`backend/security.py`, `backend/api/deps.py`) — [backend/security.py](https://github.com/quyda2004/AI_badminton/blob/main/backend/security.py)
  - Hết hạn 60 phút, `role` (user/admin)

### Multi-service JWT (chung SECRET_KEY)
- **badminton** — `backend` và `ai` cùng đọc `SECRET_KEY` → ai-service tự decode JWT lấy `user_id` mà không phải call backend hỏi. Khi ai-service gọi backend → attach nguyên JWT của user → backend enforce quyền như thường. [ai_service/router.py](https://github.com/quyda2004/AI_badminton/blob/main/ai_service/router.py)
...
- **traffic** — 3 tool, `recursion_limit=6`, `pre_model_hook` cắt history 2000 token. [backend/app/services/chat_services/ChatBotAgent.py](https://github.com/quyda2004/check/blob/main/backend/app/services/chat_services/ChatBotAgent.py)
...
- **ai-coworker** (`app/engine/graph.py`, `app/engine/supervisor.py`, `app/engine/director.py`) — [app/engine/graph.py](https://github.com/quyda2004/AI_coworker/blob/main/app/engine/graph.py)
...
- **badminton** (`ai_service/gemini_client.py`, `ai_service/tools.py`) — [ai_service/tools.py](https://github.com/quyda2004/AI_badminton/blob/main/ai_service/tools.py)
...
### MCP Server (FastMCP)
- **badminton** (`ai_service/mcp_server.py`) — [ai_service/mcp_server.py](https://github.com/quyda2004/AI_badminton/blob/main/ai_service/mcp_server.py)
...
- **ai-coworker** (`app/knowledge/retriever.py`, `app/db/vector/faiss_store.py`, `app/knowledge/ingest.py`) — [app/knowledge/retriever.py](https://github.com/quyda2004/AI_coworker/blob/main/app/knowledge/retriever.py)
...
- **traffic** — 3 model train theo pipeline paper tham chiếu:
  - **Vehicle** (YOLOv8l 6-class, mAP@0.5 = 0.909 theo paper [1] Sadik et al.) — [refer_paper/paper1_vehicle_detection_VI.md](https://github.com/quyda2004/check/blob/main/refer_paper/paper1_vehicle_detection_VI.md)
  ...
  - Notebook convert model: [backend/app/ai_models/model%20N/cvt_model.ipynb](https://github.com/quyda2004/check/blob/main/backend/app/ai_models/model%20N/cvt_model.ipynb)

### ByteTrack multi-object tracking
- **traffic** — dùng qua Ultralytics `.track(..., tracker="bytetrack.yaml")`, `confidence=0.15` (thấp để giữ box confidence thấp cho ByteTrack "vớt" xe bị che). [refer_paper/bytetrack_tomtat.md](https://github.com/quyda2004/check/blob/main/refer_paper/bytetrack_tomtat.md)

### Monocular 2D positioning (đo tốc độ)
- **traffic** — `meter_per_pixel` config trong `road_setup` (0.12 m/px mặc định), `region` polygon giới hạn vùng đo tốc độ. Dùng `solutions.SpeedEstimator` của Ultralytics. Paper [3] Lian et al.: Min accuracy 95.1%, Avg 97.6%. [refer_paper/monocular_speed_measurement_tomtat.md](https://github.com/quyda2004/check/blob/main/refer_paper/monocular_speed_measurement_tomtat.md)

### OpenVINO INT8 quantization
- **traffic** — path production: `ai_models/model N/openvino models/best_int8_openvino_model`. CPU-only 9-10 FPS. [backend/app/core/config.py](https://github.com/quyda2004/check/blob/main/backend/app/core/config.py)

### EasyOCR + Gemini Vision fallback
- **traffic** (`backend/app/modules/plate_ocr_service.py` dòng 44-138) — [backend/app/modules/plate_ocr_service.py](https://github.com/quyda2004/check/blob/main/backend/app/modules/plate_ocr_service.py)
...
- **traffic** (`backend/app/services/road_services/AnalyzeOnRoadForMultiProcessing.py`) — [backend/app/services/road_services/AnalyzeOnRoadForMultiProcessing.py](https://github.com/quyda2004/check/blob/main/backend/app/services/road_services/AnalyzeOnRoadForMultiProcessing.py)
...
- **traffic** — OCR queue trong thread riêng (`_ocr_worker_loop` dòng 310-355), `violation_sync_loop` tick 3s ghi Mongo async. [backend/app/services/road_services/violation_sync.py](https://github.com/quyda2004/check/blob/main/backend/app/services/road_services/violation_sync.py)

### Hot-reload config không restart
- **traffic** (`backend/app/services/road_services/road_setup_runtime.py` dòng 372-399) — [backend/app/services/road_services/road_setup_runtime.py](https://github.com/quyda2004/check/blob/main/backend/app/services/road_services/road_setup_runtime.py)
...
- **badminton** — [backend/models/](https://github.com/quyda2004/AI_badminton/blob/main/backend/models/)
...
- **badminton** — [docker-compose.yml](https://github.com/quyda2004/AI_badminton/blob/main/docker-compose.yml)
  - `nginx` (80), `backend` (8000), `ai` (8001), `postgres` (5432), `mongodb` (27017).

### nginx API Gateway (strip prefix, proxy split)
- **badminton** — [nginx/nginx.conf](https://github.com/quyda2004/AI_badminton/blob/main/nginx/nginx.conf)
...
- **badminton** (`tests/conftest.py`) — [tests](https://github.com/quyda2004/AI_badminton/tree/main/tests)
...
### pytest-asyncio
- **ai-coworker** — [tests](https://github.com/quyda2004/AI_coworker/tree/main/tests)
...
- **traffic** — [frontend/package.json](https://github.com/quyda2004/check/blob/main/frontend/package.json)
...
- **traffic** (`backend/seed.py`) — [backend/seed.py](https://github.com/quyda2004/check/blob/main/backend/seed.py)
